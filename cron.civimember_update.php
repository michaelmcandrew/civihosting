...
WHERE cron.name='civimember_update'",'force');

while($client=mysql_fetch_object($drupal_cron_clients)){
	$exec[]="wget -O - -q -t 1 --post-file=/clients/{$client->name}/cronpostdata {$client->base_url}/sites/all/modules/civicrm/bin/UpdateMembershipRecord.php";
}
ch_exec(implode(";\n", $exec));
?>
