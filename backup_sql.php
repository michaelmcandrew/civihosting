<?php
// connect to database
require_once('include.php');
check_root();

//Sanitize the frequency
if(!in_array($argv[1], array('daily','weekly','monthly'))){
	echo "Oops, you didn't enter a valid command line variable (daily, weekly, monthly).  Expected format backup_sql.php daily|weekly|monthly [test]\n";
	exit;
}

$time_period=$argv[1];

$databases=ch_query("SELECT c.name as client, d.name as `database`, concat(c.name, '_', d.name) as name FROM `client_database` AS cd
JOIN `client` AS c ON cd.client_id=c.id
JOIN `database` AS d ON cd.database_id=d.id
ORDER BY 1 ASC", 'force');
while($database=mysql_fetch_object($databases)){
	$filename="{$database->client}.".date('Ymd-Hi').".{$database->database}.sql";
	$path="/backup/clients/{$database->client}/sql/{$time_period}";
	if(!file_exists($path)){
		echo "Creating directory $path\n";
		ch_exec("mkdir -p $path");
		ch_exec("chmod 0755 $path");
	}
	ch_exec("mysqldump {$database->name} > {$path}/{$filename}");
	ch_exec("gzip {$path}/{$filename}");	
};
?>